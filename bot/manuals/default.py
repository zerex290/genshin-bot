...
class Converter(BaseManual):
    ...
    текста прикрепленного сообщения с инвертированием символов раскладок qwerty/йцукен.
    \r\nПримечание 1: присутствует маловероятная возможность некорректной конвертации символов, присутствующих в обеих \
    раскладках.
...
class Attachments(BaseManual):
    ...
    ранее картинку.
    \r\nПримечание 1: бот не сможет переслать больше 3 изображений за раз. Эти ограничения наложены Вконтакте.
...
class RandomTag(BaseManual):
    ...
    \r\nПримечание 1: опции можно комбинировать.
...
class RandomPicture(BaseManual):
    ...
    \r\nПримечание 3: имеется возможность искать NSFW изображения, если вы оформили подписку Vk Donut на сообщество.
    \r\nПримечание 4: имеется возможность фильтровать изображения по минимальному количеству лайков, которое они имеют \
    на сайте.
    \r\nПримечание 5: при включении интерактивного режима поведение команды существенно меняется, \
    подробности в статье: https://vk.com/@bot_genshin-artsearch-interactive-mode
    \r\nСинтаксис: !пик [опции] [количество изображений] [теги]
    \r~~п > текущая помощь по команде
    \r~~нсфв > поиск изображений с рейтингом 18+
    \r~~л [кол-во лайков] > поиск изображений только с количеством лайков большим, чем указанное в значении опции
    \r~~и > интерактивный режим поиска
    '''
